...
2. **Seal it** (from the repo, or wherever the PAP CLI is):
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/dist/pap-cli.mjs" seal openai --max-reads 10 --days 7
   ```

   It asks for the value with a hidden prompt (or reads it from stdin: `pbpaste | node "${CLAUDE_PLUGIN_ROOT}/dist/pap-cli.mjs" seal openai`). The key is encrypted on their laptop in two layers — their phone's and the agent's — and the relay only receives ciphertext.
